...
def validate_config():
    ...
    with open(os.path.dirname(__file__) + "/config_schema.yaml") as file:
        ...
        # Check if the values are valid
                if (licenseType in valid_licenseTypes or not licenseType) and \
                (product in valid_products or not product) and \
                (vendor in valid_vendors or not vendor):
                ...
                pi_logger.error("Warning: All three variables must be set together or be null/empty together.")
                sys.exit(1)